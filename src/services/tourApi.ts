...
class tourApi {
  getListTours = async () => {
    return await authAxios(`${process.env.REACT_APP_API_DOMAIN}/tours`, {
      method: 'GET',
      headers: {
  ...
  getFeaturedTours = async () => {
    const res = await authAxios(`${process.env.REACT_APP_API_DOMAIN}/tours`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  getFilteredTours = async (price, star, vehicle, type, numberOfPeople) => {
    ...
    if (vehicle != 0) {
      filteredTours = filteredTours.filter((tour) => tour.vehicle == vehicle);
    }
